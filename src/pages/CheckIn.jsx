...
export default function CheckIn() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      ...
      try {
        const res = await axios.get(`https://backmeup-backend-upload.onrender.com/api/logs/${user.uid}`);
        setLog(res.data);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await axios.post("https://backmeup-backend-upload.onrender.com/api/checkin", {
        uid: user.uid,
        note: note
